...
## Update (2026-07-18): noise-management as the diagnostic layer + two reproduction-driven fixes

The `values-diff` is the DIAGNOSTIC underneath the perceptual gate (DOC-19): once a drift-aligned crop
looks wrong, it names WHAT differs so the framework gap can be closed. Noise-management capabilities
added so its output is legible enough to drive that loop (gigabyte 211 → 112 counted defects):

- **Position is derived, not counted (REQ-64).** Absolute (x,y) is the cumulative integral of gap/size
  — pure shadow; one upstream spacing cause drifts everything below it. Reported for drill-down,
  excluded from the headline count.
- **Text-run box-`size` is a false positive (REQ-64).** A left-aligned heading's box is block-full-
  width vs the reference's fit-content — invisible (same glyphs/position). The faithful text signal is
  `renderedTextBox` (glyph extent). Box-size compared only for non-text elements (fields/panels).
- **Cause clustering — `values-diff --clusters` (REQ-76).** Rolls counted defects into ranked CAUSES,
  each tagged fix / review / accept, viewport-aware. "Do I care about Type-B" → a per-cause decision.

### Two reproduction-driven framework fixes this loop
- **Overlay wordmark anchoring (REQ-75).** A positioned wordmark placed by `--fc-x` used a VIEWPORT
  percentage, so it drifted off the hero's centered content column across widths. Added
  `Position.anchor: 'column'` → `left: calc(max(0px,(100% - var(--container-6xl))/2) + inset)`,
  reproducing the hero's left edge at every width. Generalizes REQ-52; any positioned element can anchor.
- **Card body margin (REQ-77).** A card's markdown `<p>` default margin sits inside the body flex item
  and ADDS to the card's flex `gap`, doubling the title→body spacing (ref 12px, ours 28px). Zeroed the
  body paragraphs' outer margins.

### ⚠ Framework CSS caveat: `:global()` is a DEAD no-op here
Module `<style>` blocks are collected as RAW text (styles.ts `getModuleCss`) — there is NO Astro
scoper in this path, and elements carry no scope hash. So `:global(...)` is emitted LITERALLY into

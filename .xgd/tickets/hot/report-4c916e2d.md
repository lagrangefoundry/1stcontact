...
title: 'Capability-Intent Alignment: Site Storage Port: One Async Store Behind Every
  Edit (level=uat)'
created_by: xgd
created_at: '2026-08-20T18:21:43.433474+00:00'
updated_at: '2026-08-20T18:21:43.433474+00:00'
completed_at: null
...
Tenth uat-level pass (`previous_attempt_count = 9`). REPORT-2475, 2477, 2479, 2481, 2483, 2485,
2487, 2489 and 2491 each raised the same 5 violations + 3 warnings + 1 needs_review; all nine fix
loops (REPORT-2476, 2478, 2480, 2482, 2484, 2486, 2488, 2490, 2492) applied **zero** findings, the
last six recording `progress_made=false`.

**Every finding below was re-derived from source in this call.** Nothing was carried forward on
trust from REPORT-2491. Read this pass: all 11 AC bodies pulled from the ticket store via
`ticket get --json` (the plain view truncates and `list` returns no body); `main`'s
`tests/reconciliation-site-storage-port.test.ts` (711 lines) and `…workers.test.ts` (98 lines),
extracted with `git show` and read at every line cited below;
`main:tests/test_UAT_FC_REQ-142_site_store_port.test.ts:105-122`; AC-1033
(`acceptance_criterion-ae33f0ab`) in full. Every absence claim re-established with `git grep -a`
— text mode forced, per STORY-118's documented survey hazard (`builder.ts` / `fidelity.ts` carry
NUL bytes as cache-key separators and are otherwise silently skipped as binary).
...
|---|---|---|
| HEAD | `git rev-parse HEAD` | `bcdfd129d` (advanced from `433ea8a6f`) |
| main | `git rev-parse main` | `bda6c9939` — unchanged across all ten passes |
| merge-base | `git merge-base HEAD main` | `0f44ef1ba` — unchanged |
| mutation since attempt 9 | `git diff --stat 433ea8a6f HEAD` | **two ticket files only** — `comment-13cb7682`, `report-60c4f500`. No source, no tests |
| port test files @ HEAD | `git ls-tree -r --name-only HEAD tests \| grep -iE 'site-storage\|site_store'` | **empty** |
| port test files @ main | same over `main` | `reconciliation-site-storage-port.test.ts`, `…workers.test.ts`, `test_UAT_FC_REQ-142_site_store_port.test.ts` |
...
| AC bodies last edited | `git log` over all 11 AC ticket files | `f334fed90`, 2026-08-20 09:32:22 -0700 (16:32 UTC) — the ac-level repair; **nothing since** |
...
`story_kind=feature`) carries `intent_uid = bundle-77b28def` and 11 ACs, all `status=active`, all
`kind=behavior`, all `regression_only=false`, none carrying `uat_coverage` (re-confirmed across all
11 this pass).
...
(`git grep -a -n -E "test_UAT_AC13" main -- tests` — AC1321–AC1329 present, **no AC1353 / AC1354
anywhere**). Where an AC's substance exists only under a free-coded name (`UAT_FC_REQ-14N …`),
that is **recorded but not counted as coverage** — the matrix links a test to an AC by that
convention and by nothing else.

| Element | UAT(s) found | Outcome |
|---|---|---|
| AC-1321 `acceptance_criterion-d4cc3712` | `test_UAT_AC1321_storage_answers_every_question_totally_held_and_unheld` — `…test.ts:126` | **gap** — the `asked` array (`:135-143`) carries 7 verbs (`hasDraft`, `readSiteJson`, `readPages`, `listAssets`, `readAsset`, `counter`, `loadDraft`); AC-1321 enumerates 10 non-`write` questions. `appendChange` / `changesSince` / `pendingChanges` are called by **no test in the repository** (finding 3). `loadDraft`'s reported-errors branch unasserted — `:164-172` covers only `ok: true` (`:166`) and the stamp (`:168`, `:172`) (finding 8) |
| AC-1322 `acceptance_criterion-f713cba6` | `…test.ts:197` | aligned — bytes both directions (`:225-231`), key carries no separator (`:214-217`), load order = sort order (`:218-220`), write + remove as bytes (`:248-251`); driven over both backends via `SITE_BACKENDS` (`:198`) |
| AC-1323 `acceptance_criterion-44c1d962` | `…test.ts:257` | aligned — palette rename (`:262-274`), page removal (`:277-305`) and copy edit each through `recordingStore`, exactly one write, contents matched member by member |
...
| AC-1326 `acceptance_criterion-d08eae5f` | `…test.ts:460` | aligned — CLI surface driven through `run([...argv,'--json'])`; missing-source refusal `NOT_FOUND` + path + hint (`:555`); the same refusal through `handleBuilderRequest` as a 400 carrying the same three fields |
...
authored against the AC set as it stood when BUNDLE-19 merged (`b18b859d7`). The ac-level fix loop
then ran repairs that **added AC-1353 and AC-1354, widened AC-1321, narrowed AC-1327 and widened
AC-1329** — all after the tests were frozen. Confirmed this pass from the ticket files' own git
history: every one of the 11 AC files was last written at `f334fed90`, 2026-08-20 16:32 UTC, and
none has been touched since. Findings 1–5 map one-to-one onto those five edits. No `code-issue` was
...
**The loop is provably non-convergent, and that is now the headline.** Finding 9 gates all of it and
has survived ten passes. Nothing in findings 1–8 is disputed or deferred on its merits; they are
deferred on the branch. This check is deterministic and its inputs have not changed — the only
mutation in this worktree since attempt 9 is two ticket files, and `main` has not moved from
`bda6c9939` across any of the ten passes. An eleventh iteration against this tree will reproduce
this report again. The fix side has no lever it has not already tried and correctly rejected
(authoring red-at-collection tests; writing to the `main` worktree as an unscoped cross-branch
change; setting `uat_coverage`, which is `check`/`fix_uat_coverage`'s field and not this loop's to
write; or `ac-edit`ing intent to fit the missing tests, which inverts the source of truth).

**The mechanism buying the repetition is worth an operator's attention in its own right.** The inner
loop's `progress_made=false` stuck signal is being consumed by the outer `fix_uat_validation` →
`check_uat_validation` cycle without escalating. That is why ten identical passes have been
purchased against unchanged inputs, and it will keep purchasing them. It sits outside this scope
path, so it is recorded here rather than investigated.

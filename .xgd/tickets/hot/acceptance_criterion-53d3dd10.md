...
updated_at: '2026-09-04T05:17:29.350323+00:00'
completed_at: null
...
- The bytes are present in the platform's **material store** at an address composed of the account
  the store handle is scoped to and the attachment record that owns them — the account's own
  namespace, not a shared one.
- The **same address holds nothing in the store the public site is served from.** The bytes are not
  there in any form: not at that address, and not under any other account-scoped address the material
...
return an object at the account-scoped address of the owning attachment record, and the public site's
store must return nothing for it. Because both stores are real, the check proves where the bytes
actually landed rather than what the configuration says about it.

...
Three implementations are live and current at the same time, none a legacy mode and none
detected at runtime — the one chosen at start-up is the one the whole process uses:

- the operator's git-tracked site tree on their own machine,
- a store with no filesystem behind it at all, and
- a store whose definitions are database rows and whose asset bytes are objects, scoped so a
  handle can only ever see one account.
...
property that is checked every time the suite runs. One body of storage assertions is run
against all three, which is what keeps them from drifting apart.

...
updated_at: '2026-08-24T21:42:43.463102+00:00'
completed_at: null
...
`apps/control-app/wrangler.toml` declares no `[observability]` block, at the top
level or under `[env.production]` (which inherits nothing). The account
therefore retains no invocation logs, only aggregate counters — which is why
this took inference from source plus a plan page rather than a log read. Worth
adding; config-only, no code.

...
The declaration is the thing actually in force, not a document describing what the build separately decides. Every declared value — the knowledge base's description, its ranking weight, and the predicate that says which documents belong to it — is what the build and the reader use, so editing the declaration changes what gets built and how it ranks.

The declaration is **authored data**: it is written once when absent so a fresh checkout can build without hand-writing a file, and a build never overwrites an existing one. A tuned description or an adjusted weight survives every rebuild.

...
## Update (2026-07-08, pass 3): read the per-section diff at FULL resolution; the pixel diff *does* show "finish" (weight / shape / shadow / opacity)

Context: the joyfulculinary hero passed my own eyeball review as "strong." The operator, reading the same `1c diff`, immediately named seven finish defects — scrim too transparent, nav too high, logo too small / mis-placed / **no shadow**, main heading weight too heavy, divider too long/thin/faint, section headings **far** too heavy, "Learn More" button rounded where the reference is square. **Every one was visible in the pixel diff.** The failures were in how I *read* the gate, plus one broken measurement and one real capture gap. Each below is a constraint **with its reason**.

### Lesson 1 — read the diff per SECTION at native resolution, never the whole-page thumbnail
**Why:** the whole-page overlay (~4744px tall) displayed as a thumbnail shrinks the hero to a sliver where doubled elements read as sub-pixel jitter. I called the hero "mostly aligned" from exactly that view. **Crop each section (`1c crop <img> --box 0,<y>,1280,<h>`) and view its `-diff` at full size.** At 1280×800 the hero's doubled headings, offset nav, missing logo card, and button-corner deltas are unmistakable. (Extends the earlier "read the overlay" note with: *at the right zoom* — a scaled-down overlay is as blind as the mean.)

### Lesson 2 — the pixel diff encodes "finish"; learn its reading vocabulary
**Why:** I wrongly told the operator that weight / radius / shadow / opacity were "un-seeable by any mechanical check." That is true only of the *automated* `values-diff` (which doesn't record radius/shadow/opacity — see the capture gap). The **pixel diff shows all of them.** Fixed vocabulary (extends the overlay-signature list):
- **Uniform tone across a whole band** (grey where matched pixels should cancel to black) → **scrim / opacity** delta. If ours matched, untouched image areas cancel to near-black; a grey wash *is* the transparency gap.
- **Doubled element** → position offset (then measure it).
- **Two copies with different stroke thickness** → **font-weight** delta — the heavier glyph has fatter strokes, so a 600-vs-500 lights up as a halo around every letter. (Weight is *not* invisible to pixel diff.)
- **A bright rectangle / soft-edged box with nothing inside it** → a **missing card / shadow / border** (the reference's shadowed logo card lit up as a box; ours had none).
- **A bright notch at a box corner** → **border-radius** (square vs round).
- Amplify the subtle ones: `sharp(ours).composite([{input:ref,blend:'difference'}]).linear(3,0)` — the ×3 absolute-difference makes weight halos and the scrim wash pop where the thresholded overlay is faint.

### Lesson 3 — do not optimize the perceptual mean while ignoring values-diff
**Why:** I drove the mean 64→39 and read it as progress while `values-diff` was reporting **132 deltas** — including `fontSize "THE HOLISTIC APPROACH" expected 44 actual 32`, systemic `lineHeight` drift ×29, and the heading weights. The mean fell while the *finish stayed wrong* because the mean is structurally blind to type finish **and** I was not reading the overlay at resolution. This is Goodhart's law: I optimized the proxy I could move. **Heed every values-diff delta each iteration; a low mean over a hero proves almost nothing.**

### Lesson 4 — validate the measurement before you trust it
**Why:** my "scrim matches" (ours 138 vs ref 139) came from a broken helper — **sharp's `.stats()` ignores a queued `.extract()`** and averages the whole image, so three different patches returned identical numbers. Materialize the crop first: `sharp(f).extract(box).png().toBuffer()` → `sharp(buf).stats()`. **Sanity gate: three distinct patches returning identical values means the tool is lying — stop and fix it.** Remeasured correctly, ours was lighter by Δlum 9–22 everywhere (scrim ≈0.55 vs the reference's ≈0.68).
...
### The standing per-section checklist (run every iteration)
1. `values-diff` clean, or every delta explained — size / weight / colour / spacing / leading.
2. Crop the section; **read its `-diff` overlay at full resolution** with the vocabulary above (tone → scrim; doubling → position; stroke thickness → weight; empty box → shadow/card/border; corner notch → radius).
3. Amplify (`×3 abs-diff`) for scrim / weight / shadow.
4. Measure — your own output included; validate the measurement tool; never infer px from ghosts.
5. Only then judge by eye what neither gate encodes.


## Update (2026-07-08, pass 4): the capture bundle IS the asset source — check it before declaring an "asset gap"

I told the operator four things were "asset-blocked — supply the source files": the Offerings
card photos, the How-It-Works icons, a dark logo, and a light Lato. **All four were already in
the capture bundle.** The operator's challenge — "surely the assets come from the download; if
not, that's a bug" — was correct.

**Root cause:** `1c capture` mirrors *every* asset the page loads into `<bundle>/assets/`
(images, icon **fonts**, and **all font weights**). The sandbox repro, though, had only been
populated with **3 of the ~7 images**, **1 of 3 Lato weight files**, and **none of the icon
fonts**. I built config around what was in the sandbox and then wrongly concluded the rest were
"unavailable." The capture was complete; the *import into the sandbox* was not.

**The rule:** before calling anything an asset gap, **`find <bundle>/assets/` and read the
capture's CSS** (`lato.css` etc.). Specifically:
- Photos come as the site's own files (`9.jpg`, `10.jpg`, `IMG_8708…`); map them to sections by
  **content** (view them) — DOM/srcset order is unreliable. Downscale the huge originals
  (1200–1536px, up to 2.3MB) to ~600px before use.
- **Icons are usually an icon font** (Font Awesome `fa-solid-900.woff2`, Elementor `eicons.woff2`)
  with glyph classes in the HTML (`fa-clipboard-list`, `fa-comments`, `fa-check`), *not* image
  assets — reproduce by loading the font + mapping glyphs.
- **Web-font weights are separate static files** (Google Fonts): `lato.css` declared Lato 100/300/
  400, but only one `.woff2` was imported → the light lead couldn't render. Import the weight file
  you need (my "Lato is a static single-weight file" finding was an artifact of the under-import).

**Tooling follow-ups this surfaced:**
- The capture→sandbox scaffold should import **all** captured assets (or surface which captured
  assets are unused), so a repro can't silently run on a subset.

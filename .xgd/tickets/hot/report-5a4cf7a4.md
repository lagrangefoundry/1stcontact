...
| 5 | warning | exclusivity | AC-1036 (acceptance_criterion-46e9debf) vs AC-978 (acceptance_criterion-53c66f17) | ac-edit | The second paragraph of AC-1036 re-states the rendered-channels probe of AC-978: same three traversal forms (plain, percent-encoded dots, encoded separators), same target, same 'root:' secret, same not-ok, status at least 400, and no-contents assertions — tests/reconciliation-builder-request-time-render.test.ts:429-461 against tests/reconciliation-builder-workspace-origin.test.ts:195-244. Same scenario in the same shape, which the prompt distinguishes from acceptable multi-shape coverage. The genuinely distinct content of AC-1036 is the *addressing* claim (directory address to home page, extensionless to page, the two panel addresses unchanged) plus the unknown-page and unknown-site probes AC-978 does not make | Narrow the confinement clause of AC-1036 to the claims AC-978 does not make (a page the channel does not contain, a site the store does not hold, never answered from a neighbour) and defer the traversal probes to AC-978 — or state in AC-1036 why they are deliberately re-run against the request-time path |
| 6 | warning | consistency | AC-964 (acceptance_criterion-5286c04b) | ac-edit | The criterion of AC-964 names five things reachable from one host — 'the workspace document, its components, **its browser source**, the rendered channels and the workspace operations' — but its Verification enumerates only four routes and the browser source is not among them: tests/reconciliation-builder-workspace-origin.test.ts:609-614 compares /, a component module, /preview/alpha/draft/ and /api/sites. /builder/main.js and /builder/builder.css are served (they appear in the AC-977 and AC-978 sweeps) but are never verbatim-forwarding-compared. The route list mirrors the narrower enumeration in STORY-99 — the same narrowing that produced findings 2 and 3. Also recorded as REPORT-2100 warning 3; repeated here because the element under assertion is an AC | Widen the verbatim-forwarding route list of AC-964 to include the browser-source tree, and whichever of the /framework/edit-client.js and /api/copy routes survives findings 2 and 3 |


| 7 | warning | consistency | AC-965 (acceptance_criterion-6fb2bebc) | story-body-edit | AC-965 (unconfigured vs unreachable origin — distinct statuses, install command named, attempted address named) follows from the CAP-85 body ('An unconfigured origin and an unreachable one are distinct, self-explanatory failures rather than a blank page') but from **no bullet of STORY-99**. The single-origin bullet enumerates only what is reachable; Technical Context describes the Node-origin-plus-front arrangement without naming either failure mode. The AC is correct, active and evidenced (tests/reconciliation-builder-workspace-origin.test.ts:641); the defect is that the story body it hangs from does not express the behaviour. Third instance of the same root cause as findings 2, 3 and 6 | Add the two origin failure modes to the in-scope enumeration of STORY-99. No AC change |

...
updated_at: '2026-07-30T19:02:32.178746+00:00'
completed_at: null
...
fields:
  ...
  - working_sha: 3714436c4fa586ad0a2391bc65ded4f3b6b8c290
    reconcile_sha: null
    main_sha: null
  - working_sha: e203df7067fb1dacf967860bddddfbef9711bbfc
    reconcile_sha: null
    main_sha: null
  - working_sha: 91702915dee2a6f3980a296e4a36039931cb3960
    reconcile_sha: null
    main_sha: null
  - working_sha: de973d5e1f2657c635680374c6e9be9445ab7b44
    reconcile_sha: null
    main_sha: null
  version: 0.1.4
---
...
It applies to five bands — the three that paint a visible grid:
...
`#problem` ("The problem") and `#close` ("Evidence, not promises") carried a
full-bleed `pattern` grid at `#8b5c2a1a`. **The operator asked for those grids to
be removed** — they were never meant to be noticed, and the accent was the first
thing that drew the eye to them. They now carry an **invisible grid** instead: a
`pattern` at `#8b5c2a00` (fully transparent) that paints nothing, while the accent
redraws that same pattern in teal. So the grid exists ONLY under the cursor — the
resting band is flat cream (0 of 4080 sampled edge pixels differ from its fill)
and ~1375 px light up under the pointer.

Spacing there is 24px, not the 48px those bands used to carry: with the base
invisible the spacing stops being a design constraint and becomes purely an effect
parameter, and at 48px only ~4 lines fall inside the region — a crosshair rather
than a patch of grid.
...
`#contract` (asset grids, colour `#2e86a3`) and on `#problem` / `#close`
(invisible 24px `pattern` grids, colour `#2e86a3a6`). All five:

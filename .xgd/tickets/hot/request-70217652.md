...
updated_at: '2026-07-30T18:51:45.658177+00:00'
completed_at: null
...
It applies to the three bands that paint a grid:
...
thing that drew the eye to them — so the `pattern` axis and its accent are gone
from both bands. They render as plain cream.
...
`storage/sites/xgd/draft/pages/home.json` — `pointerAccent` on `#hero`, `#how` and
`#contract`: `radiusPx: 95` (~190px across), `softnessPx: 14`, `roughness: 0.65`.

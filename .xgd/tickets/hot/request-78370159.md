...
updated_at: '2026-09-01T00:42:19.782228+00:00'
completed_at: null
...
- **AC-1064** ("only the toolbar chooses a site") asserted `querySelectorAll('select').length === 1`
  ...
  lists site slugs.
- **[[REQ-163]]'s blob-addressing criteria** asserted `t/<tenant>/blob/<sha>` and
  "the same file twice is ONE blob". Both were true of the component's previous
  content-addressing and are not true of the current one (see above). Restated:
  one record owns one blob, both under this tenant's prefix, identical content
  still hashing identically.

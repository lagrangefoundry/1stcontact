...
updated_at: '2026-08-31T21:47:36.428242+00:00'
completed_at: null
...
4. **Asset upload flow** — **answered** ([[REQ-161]]). Both routes, not a choice between them: drag onto the chat in the `site` tab (the conversational path — the AI asks for a logo, the client drops one in) *and* drag onto a **Library tab** (the deliberate path). Both raise the same full-screen overlay, whose drop areas are **roles, not file types** — "put it on the site" versus "just for you to read". Sorting by file type would ask the client for something the content type already tells us while leaving unasked the one thing we cannot infer: the same JPEG may be a hero photograph destined for publication or a competitor screenshot that must never be published ([[DOC-38]] §4.2). The Library itself is `webui/split` + `webui/list-detail` over the client's material tickets, reusing the existing field editors rather than growing a second editing vocabulary.

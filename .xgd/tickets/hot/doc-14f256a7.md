...
updated_at: '2026-07-21T23:59:59.197654+00:00'
completed_at: null
...
## REQ-85 update — the tool surface after the pivot (configure capabilities + author L1)

The canonical pipeline (**chat → AI → structured-diff → state → renderer**) and the
in-browser preview are unchanged. What the AI's tool surface *edits* changes with the
pivot ([[DOC-24]]):

- **Author L1 directly** for all layout/appearance — the typed [[DOC-23]] substrate,
  under the safety envelope. This is where taste lives; there are no aesthetic dials.
- **Configure behavior modules** ([[DOC-25]]) for behaviour: set a capability
  instance's typed `config` (behavioural params) and fill its named `slots` with
  **L1 subtrees** (the presentation). The AI *configures* a capability; it never
  writes its code.

So the allowed-edits surface is now **(a) L1 tree edits** and **(b) capability
`config` + `slots` edits** — both structured, both validated (`validateL1`,
`validateBehaviorInstance`), never raw CSS/HTML ([[DOC-2]]). The four-layer
mechanical gate ([[DOC-7]]) still applies; the renderer the preview invokes is the
same one `tools/generate` ships, including the folded capability chrome CSS
(`getModuleCss`) and client behaviour (`getModuleClientJs` → `capabilities.js`).
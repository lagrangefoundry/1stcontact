...
updated_at: '2026-09-02T00:42:13.741647+00:00'
completed_at: null
...
## BLOCKED — this ticket's conclusion collides with BUG-45

The operator asked for the registry and all its code and doc references to be
removed. That cannot proceed as asked, for two reasons found while surveying the
blast radius.

### BUG-45 already specifies the opposite, and is already branched

BUG-45 ("Chat upload: a file dropped on 'Put it on the site' is unusable to the
assistant") is `status: free_coding` with `free-BUG-45` cut. Its required
behaviour includes:

> 1. **Promotion registers.** … bytes stored, registry entry created,
>    `site.json` validated …
> 2. **The description becomes the alt text.** The material ticket's AI-written
>    title lands on the registry entry …
> 5. … `registered` remains reportable — it is a true fact about the file …

Deleting the registry contradicts 1, 2 and 5 directly. No code has been written
on that branch yet (`xgd-working..free-BUG-45` carries only ticket commits), so
the collision is one of specification, not of merge.

### A correction to this ticket's own fault 4

BUG-45 read the live miniflare state before it was reset and found
`material-a4932624` carrying `role: "reference"`, `republishable: false`.
`placeOnSite` (`router.ts:439`) returns at its first line on `role !== 'site'`,
so **the client's logo never reached the site's asset folder at all**. It was
not an unregistered asset; it was not on the site.

Fault 4 above therefore overstates the case: the promote path's failure to
register is real and confirmed independently, but it is not what happened to
this client's logo, and the files `list_assets` did show were not it.

### What the collision actually turns on: where alt text lives

BUG-45's behaviour 2 is the one live use for the registry that this ticket's
"the metadata is dead" analysis under-weighted — the material's AI-written
description becoming the asset's alt text.

But the registry's `alt` is **write-only today**. Nothing propagates it to an
L1 node: `imageHandles` (`edit.ts:1909`) maps `listSiteAssets` to `.src` alone,
and `imageChoices` (`l1/edit.ts:997`) receives handles, not entries. Writing the
description onto a registry entry would put it in a field no renderer and no
picker reads, and would need further plumbing to become visible.

The node is where alt already lives and is already required
(`l1ImageSchema.alt: z.string()`), and the assistant can read the material's
description through `KnowledgeGet` and set it there directly, with no plumbing
at all.

### Options

- **A — delete the registry, rewrite BUG-45.** BUG-45's intent survives whole:
  with no registry there is no unregistered state, so its behaviours 5 and (by
  construction) 1 are satisfied by the deletion; behaviour 2 is re-aimed at the
  node's own `alt`; behaviours 3, 4 and 6 are untouched.
- **B — keep the registry, do BUG-45 as written,** and additionally build the
  propagation that makes registry `alt` reach a node.
- **C — both, sequenced:** BUG-45 as written first, deletion as later cleanup.
  Pays for plumbing that the deletion then removes.

Awaiting the operator's decision. No code written.

...
created_by: xgd
created_at: '2026-07-24T07:32:16.577266+00:00'
updated_at: '2026-07-24T07:32:16.577266+00:00'
completed_at: null
...
Both stories in CAP-65 draw from a single reconciled intent bundle. No `updated_by`
chain on either story — the capability tree has exactly one originating intent.
...
| — REQ-58 (in bundle) | reconciled (via bundle) | — | Multi-viewport capture / persisted viewport ladder + per-viewport reference screenshots that the `--size` diff work reuses | YES — CAP-65's capture dependency |
| — REQ-59 (in bundle) | reconciled (via bundle) | — | Text-fill gradient stop positions in capture/values-diff | NO — belongs to gradient-diffing capability, not CAP-65 |
| — REQ-62 (in bundle) | reconciled (via bundle) | — | Panel background gradient capture/render/diff | NO — belongs to gradient-diffing capability, not CAP-65 |

CAP-65's slice of the bundle is REQ-61 (size-aware + responsive-diff) resting on
REQ-58's multi-viewport capture. REQ-59 and REQ-62 share the bundle but land in a
different capability (gradient diffing) and are out of scope for this tree.
...
| 2 | info | consistency | STORY-77, STORY-78 | — | REQ-61 prose proposed `--size desktop\|tablet\|phone`; both stories and the shipped CLI use `mobile\|tablet\|desktop`. This is not drift: `perceptual.ts:388` attributes the `mobile\|tablet\|desktop` vocabulary to REQ-61, so the reconciled shipped reality settled the token as `mobile`. Stories are grounded in the implemented interface (STORY-78 carries an explicit `--ref`-vs-slug divergence note in the same spirit). | none |
...
references, no retired behavior lingering. The only observation worth carrying
forward is the `phone`→`mobile` vocabulary settlement (finding #2): the REQ prose
and shipped token differ, but the shipped code explicitly attributes `mobile` to
REQ-61, so the matrix correctly tracks the reconciled reality rather than the
pre-implementation prose. No action.

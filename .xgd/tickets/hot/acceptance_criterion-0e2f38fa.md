...
updated_at: '2026-09-10T17:59:39.371850+00:00'
completed_at: null
...
**scaling** adjustments. The hue shift and the blur **are** the axis — same
name, same number, nothing converted — because a control is a projection only
where it is one, which is the rule already at work where italic projects onto
the parameter beneath it.
...
assert the save succeeds and the stored region carries each **scaling**
...
controls back to its own identity and assert the region carries no colour

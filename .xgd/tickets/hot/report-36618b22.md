...
title: 'Capability-Intent Alignment: Site Storage Port: One Async Store Behind Every
  Edit (level=uat)'
created_by: xgd
created_at: '2026-08-20T17:45:06.625283+00:00'
updated_at: '2026-08-20T17:45:06.625283+00:00'
completed_at: null
...
Sixth uat-level pass (`previous_attempt_count = 5`). REPORT-2475, 2477, 2479, 2481 and 2483 each
raised the same 5 violations + 3 warnings + 1 needs_review; all five fix loops (REPORT-2476, 2478,
2480, 2482, 2484) applied **zero** findings, the last two recording `progress_made=false`.

**Every finding below was re-derived from source this pass**, not carried forward on trust. What
was actually read this pass: all 11 AC bodies from the ticket store; `main`'s
`reconciliation-site-storage-port.test.ts` (711 lines) and `…workers.test.ts` (98 lines) extracted
with `git show` and read at the cited lines; `main:tests/test_UAT_FC_REQ-142_site_store_port.test.ts:95-140`;
and each absence claim re-established with `git grep -a` (text mode forced — STORY-118's survey
hazard: `builder.ts` and `fidelity.ts` carry NUL bytes and a plain recursive grep skips them
silently). All eight actionable findings survive re-verification unchanged.

**Nothing moved between passes.** `main` tip is still `bda6c9939`; this worktree's HEAD is
`f327a5292` (ticket/report bookkeeping only, advanced from `32c65d814`); merge-base with `main` is
still `0f44ef1ba`. `git grep -a -h -o -E "test_UAT_AC13[0-9][0-9]_…" main -- tests` yields
AC1300–AC1342 with AC1321–AC1329 present and **no AC1353 / AC1354 anywhere**.

## Two refinements this pass adds

Both narrow the eventual repair; neither changes a verdict.

**1. Finding 6 splits cleanly, and half of it is already correct.** REPORT-2483 recorded AC-1328's
bullets 3 *and* 4 as "asserted at `…test.ts:630-654`", inside AC-1329's test. Re-reading both
files, bullet 4 has two halves and they sit in different places: the **binding-names** half is
proved inside AC-1328's *own* test (`…workers.test.ts:40-41`, `expect(DB).toBeDefined()` /
`expect(SITES).toBeDefined()`, asserted against the live runtime rather than a config file — the
file's own comment at `:35-39` says so deliberately). Only the **compatibility-settings** half sits
in AC-1329's test (`…test.ts:630-636`). So the misfiling under finding 6 is bullet 2, bullet 3, and
half of bullet 4 — not all of 3 and 4.

**2. Finding 4's exclusivity breach is categorical, not inferential.** AC-1033
(`acceptance_criterion-ae33f0ab`, CAP-85, `uat_coverage: pass`) is *titled* "A definition changed
outside the workspace is shown on the next request, with no render step and no restart". The
assertion at `…test.ts:585-590` is that sentence, executed. It is not merely adjacent to CAP-85's
evidence; it is the same claim under CAP-101's name.
...
| `git ls-tree main tools/generate/src/store/` | adds `assemble.ts`, `fs-store.ts`, `journal-model.ts`, `journal.ts`, `memory-store.ts`, `site-store.ts` |
...
**No test was executed.** The modules under test are absent here and this check is read-only. Every
finding is a claim about what a test *asserts* versus what its AC *requires*, established by
reading both. **None is a claim that a test fails.**
...
|---|---|---|---|---|
| BUNDLE-19 (`bundle-77b28def`) | `free_and_reconciled` | 2026-08-18 | The story's intent; `merged_at_commit = b18b859d7` (re-verified this pass). Nine source tickets, of which only REQ-141 and REQ-142 carry a storage or test-runtime surface | YES |
...
| AC-1322 `acceptance_criterion-f713cba6` | `…test.ts:197` | aligned — bytes both directions, key carries no separator, load order = sort order, surface listing names no location; driven over both backends via `SITE_BACKENDS` (`:198`) |
...
| AC-1328 `acceptance_criterion-c8728ae8` | `test_UAT_AC1328_workers_marked_file_runs_in_workerd_with_real_bindings` — `main:tests/reconciliation-site-storage-port.workers.test.ts:30` | **partial** (finding 6) — bullet 1 fully proved (UA `:32`, `sqlite_master` read-back `:51-56`, PK enforced by the engine `:68-72`, R2 server-computed size + etag `:81-82`, metadata round trip `:87`); binding names proved `:40-41`. Bullet 3 and the compat half of bullet 4 are asserted inside **AC-1329's** test (`…test.ts:630-636`, `:647-654`); bullet 2 only under an FC name (`main:tests/test_UAT_FC_REQ-141_project_routing.test.ts:25`) |
...
| 9 | needs_review | coverage | `capability-c4c7a854` (repair of findings 1–8) | — | **Escalation, not intent ambiguity — sixth consecutive pass, zero movement.** All eight findings above are `uat-add` / `uat-edit` against `tests/reconciliation-site-storage-port.test.ts`, which does not exist in this worktree: the port landed on `main` at `2b902ead0` (2026-08-20 12:21:02 UTC) / `b18b859d7` (12:49:19 UTC), **11h38m after** regression `cb0dad9c`'s branch was cut at `0f44ef1ba` (2026-08-20 00:43:02 UTC — re-derived in UTC this pass from `%cI`). Authoring the tests here would import absent modules, failing at collection and adding a knowingly-red suite to the branch whose sole purpose is to gate a fast-forward of `xgd-stable` — red against *correct* code. Re-verified directly this pass: `HEAD:tools/generate/src/cli/edit.ts:1-2,24` is `import { copyFileSync, writeFileSync } from 'node:fs'`, `import path from 'node:path'`, `import type { Root, StoreContext } from '../store'` — so **all three** of AC-1353's first bullet's prohibitions are genuinely false in this tree, not merely unevidenced. Five fix loops applied 0 of 8 and were right not to | Operator decision, one of: **(c)** run `check_uat_validation` + `fix_uat_validation` for this capability on a branch at or past `b18b859d7`, where all eight findings are both actionable and runnable; **(b)** scope `capability-c4c7a854` out of regression `cb0dad9c` — noting that the *check* is runnable here and its result is sound, so (b) only relocates the *repair*; **(a)** resync `regression-cb0dad9c` past `b18b859d7`, which makes them actionable here but changes what the regression is testing mid-run. **(c) recommended**, then (b); (a) remains least attractive |
...
`createL1Toolbox`. This also explains why the 12 `createL1Toolbox` call sites in `main:tests` are
not evidence for AC-1354 even though several pass an options object: whatever they pass, the store
is overridden.
...
6 is cheaper than REPORT-2483 implied — the binding-names half of AC-1328's bullet 4 is already in
the right test.

**Blocked-on.** Finding 9 gates all of it and has now survived six passes. Nothing in findings 1–8
is disputed or deferred on its merits; they are deferred on the branch. A seventh iteration against
this tree will produce this report again — the loop cannot converge without the operator decision.

...
updated_at: '2026-09-02T23:25:36.525732+00:00'
completed_at: null
...
5. **A token that cannot list accounts is diagnosed, not misreported.** Discovery
   ...
   envelope with a 200). That refusal must produce an error naming the narrow-scope
   case and telling the operator to set `CLOUDFLARE_ACCOUNT_ID` explicitly — the
   one failure that would otherwise read as "the knowledge build is broken" when
   the credentials are fine and only unusually scoped.

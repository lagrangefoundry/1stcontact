...
created_by: xgd
created_at: '2026-09-10T01:26:55.236832+00:00'
updated_at: '2026-09-10T01:26:55.236832+00:00'
completed_at: null
...
- `.xgd/tickets/hot/bundle-b3b7c399.md` — UU, intent/bookkeeping ticket (rule 2e).
  ...
  - Incoming (`726b77db28`, 2026-08-28T03:57Z): `status: ready_to_reconcile`,
  ...
  `ready_to_reconcile` on its way to `free_and_reconciled`. Resolved to the HEAD
  side; no other region of the file was in conflict.
...
transition `reconciling -> ready_to_reconcile` plus the accompanying timestamp
bookkeeping.
...
by advancing through `ready_to_reconcile`. Writing the incoming values would
regress operator-owned lifecycle state on the ticket. This is the
STEP 4 / BUG-1109 redundant-commit shape, not the STEP 3 discard shape: the
incoming commit's effect is present in HEAD via a later route, so the staged
diff vs HEAD is empty and the finalize step will skip the commit.

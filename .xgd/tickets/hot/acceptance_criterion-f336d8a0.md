...
resolves that page's document-relative asset references against the published
site root, so the page loads with its stylesheet and images intact.

## Verification

Publish a rendered site and request the slash-terminated form of a page's clean
...
served URL yields the asset's address directly under the published site root —

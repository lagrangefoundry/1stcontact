...
updated_at: '2026-09-10T17:29:56.911030+00:00'
completed_at: null
...
operation — **both** the editable rendering and the plain draft rendering,
because an edit changes the page and not one rendering of it, and a change that
reached only one of them would leave the other serving an indefinitely stale
page with nothing to signal it. Each rendered output on disk therefore contains
the new words and no longer contains the old ones. The result reports which
fields changed and **where each of the two renderings was written**. Submitting
values identical to the current ones succeeds and reports that nothing changed.

...
describe('REQ-131 — a session is TOLD when the site moved under it', () => {
  ...
  it('test_UAT_FC_REQ_131_the_reminder_carries_the_signal_only_when_something_changed', async () => {
    ...
    // did.
    //
    // READ OFF EVERYTHING THE TURN SENT. The signal is the per-turn reminder's,
    // and upstream moved the reminder out of the system field and onto the tail
    // of the user message ([[BUG-83]]); a negative assertion aimed at the old
    // field would pass whether or not the signal fired.
    ...
    const primed = sentText(client.seen[1])

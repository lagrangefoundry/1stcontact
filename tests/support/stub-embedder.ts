 ...
 * reason. What the client-KB suites prove is the mechanism around it: that a
 * corpus resolves, that an index is incremental, that one account cannot see
 * another account's vectors, that an absent index reads as absent. None of those
 ...
 *
 * COUNTED, which is what makes the incremental claim measurable: `calls` counts
 * texts embedded, so a refresh that kept N documents is asserted against the
 * model's own call count rather than against a tally the implementation computes.
 */

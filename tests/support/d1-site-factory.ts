...
const MIGRATIONS = [
  ...
  // same reason as the two above, and this one earns the reason twice over: a
  ...
  // keep honest.
  () => import('../../db/migrations/0004_retire_site_slug.sql?raw'),
]
 ...
 * `describe`, because storage persists across them inside one file. `0004`
 ...
 * longer skips, because `0004` dropped that index — and fails on a column that
 ...
 *
 * `kind` IS THE MARKER BECAUSE `0004` IS WHAT ADDS IT, so its presence means

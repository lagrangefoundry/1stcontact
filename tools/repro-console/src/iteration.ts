 ...
 *
 * THE TWO ARE NOT THE SAME THING. A step's `cwd` is the tree it operates on —
 * which `storage/` it reads and writes. The CLI's own location is fixed by
 * where the console is installed. Deriving the second from the first worked
 * only because they coincide in normal use, and broke the moment anything ran
 * a step against a directory that was not this checkout.
 */

...
export async function run(argv: string[]): Promise<void> {
  ...
  switch (command) {
    ...
    case 'kb': {
      ...
      if (sub === 'status') {
        const s = kbStatus()
        // The corpus line names its two producers separately rather than
        // reporting one total (REQ-165). The projections have no ticket to be
        // counted by, so a single number says nothing about how much of the
        // corpus came from the ticket store — and a corpus whose projections
        // are missing reads exactly like one that is merely small.
        const exported = s.corpus - s.projected
        console.log(
          `corpus: ${exported} exported + ${s.projected} projected\n` +

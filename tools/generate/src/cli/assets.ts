...
export interface AssetBuildReport {
  ...
  /** The absolute path `src/generated/ai-ticketing.js` re-exports ([[REQ-228]]). */
  aiTicketingEntry: string
  /** The absolute path `src/generated/logging.js` re-exports ([[REQ-235]]). */
  loggingEntry: string
...
/**
 * The ticket surface bridge, as a module wrangler can follow ([[REQ-228]]).
 *
 * IDENTICALLY MOTIVATED TO {@link writeAiKnowledgeShim}, and the same three
 * lines: the bridge lives in the same out-of-repo shared store, so a bare
 * specifier in the Worker resolves by walking up from the importing file — which
 * finds the store from the main checkout and finds NOTHING from a linked `git
 * worktree`. Same hazard, same single resolution point, same one-line re-export
 * carrying an absolute path.
 *
 * THE PACKAGE ROOT, like the ticket store's own shim and unlike the AI
 * component's `/workers` rung. The bridge's root imports `@lagrangefoundry/ai`
 * through `/core` — the SDK-free entry point — and the one export that reaches a
 * local project (`./node`, which spawns a subprocess) is a separate subpath
 * nothing here names. There is no rung to pick.
 *
 * WHY THE IDENTITY OF `ToolboxSurface` SURVIVES THIS. The bridge extends the
 * class from `@lagrangefoundry/ai/core` while the Worker's own library arrives
 * through `/workers`; both subpaths of one package resolve to one module graph,
 * so the class the bridge extends IS the class the Toolbox checks against. That
 * is the same composition `ai-knowledge` has had since [[REQ-158]], which is why
 * this is a known shape rather than an experiment.
 */
function writeAiTicketingShim(generatedDir: string): string {
  const entry = sharedModulePath('ai-ticketing')
  fs.writeFileSync(
    path.join(generatedDir, 'ai-ticketing.js'),
    `// Generated by \`1c assets\` ([[REQ-228]]). Do not edit, do not commit.\n` +
      `// Resolves the out-of-repo shared store to an absolute path so wrangler can\n` +
      `// bundle it from any checkout, including a linked git worktree.\n` +
      `export * from ${JSON.stringify(entry)}\n`,
  )
  fs.writeFileSync(
    path.join(generatedDir, 'ai-ticketing.d.ts'),
    [
      '// Generated by `1c assets` ([[REQ-228]]). Do not edit, do not commit.',
      '/* eslint-disable @typescript-eslint/no-explicit-any */',
      ...AI_TICKETING_EXPORTS.map((n) => `export const ${n}: any`),
      '',
    ].join('\n'),
  )
  return entry
...
/**
 * What the Worker reaches for out of the ticket bridge.
 *
 * Listed rather than wildcarded for the reason {@link AI_WORKER_EXPORTS} gives.
 * Four names and no more, because this composition is deliberately narrow: the
 * surface class, the configuration builder that says which groups a session
 * gets, and the two group constants — so that `READ_GROUP` is read from the
 * declaration's own vocabulary rather than spelled `'ReadTickets'` by hand in
 * this repository.
 *
 * `WRITE_GROUP` IS NAMED AND NEVER GRANTED, and that is not a contradiction.
 * Having the constant is what lets a UAT assert the withholding against the
 * bridge's own name for the thing being withheld; spelling it as a literal there
 * would make the assertion pass the day upstream renamed the group and the grant
 * silently changed meaning.
 */
const AI_TICKETING_EXPORTS = [
  'READ_GROUP',
  'TicketToolbox',
  'WRITE_GROUP',
  'instanceConfig',
...
/**
 * What the Worker reaches for out of the logging component ([[REQ-235]]).
 *
 * Listed rather than wildcarded for the reason {@link AI_WORKER_EXPORTS} gives,
 * and this list in particular is doing more than naming imports. `RECORD_FIELDS`
 * and `KINDS` are the DIMENSION SET — [[EPIC-1]] §37 calls it the one
 * irreversible decision in that epic, because a dimension the record never
 * carried cannot be recovered afterwards at any cost. The store's columns are
 * built against it (`log.ts`'s `COLUMN_OF`), so an upstream change to it must
 * surface here as a typecheck failure rather than as a column that silently
 * stopped being written.
 */
const LOGGING_EXPORTS = [
  'DEFAULT_LEVEL',
  'DIMENSIONS',
  'IDENTIFIERS',
  'KINDS',
...
/**
 * The logging component, resolved for the Worker ([[REQ-235]], [[EPIC-1]]).
 *
 * SAME SHIM, SAME REASON AS EVERY ONE ABOVE. The package lives in the same
 * out-of-repo shared store, so a bare specifier in the Worker resolves by
 * walking up from the importing file — which finds the store from the main
 * checkout and finds NOTHING from a linked `git worktree`. One re-export
 * carrying an absolute path closes it, through the single resolution point
 * rather than a second guess at where the store is.
 *
 * THE PACKAGE ROOT, because there are no rungs: it has zero runtime
 * dependencies, no build step and nothing in it that knows what a Cloudflare is,
 * which is precisely what lets the same module serve the Worker and a test.
 */
function writeLoggingShim(generatedDir: string): string {
  const entry = sharedModulePath('logging')
  fs.writeFileSync(
    path.join(generatedDir, 'logging.js'),
    `// Generated by \`1c assets\` ([[REQ-235]]). Do not edit, do not commit.\n` +
      `// Resolves the out-of-repo shared store to an absolute path so wrangler can\n` +
      `// bundle it from any checkout, including a linked git worktree.\n` +
      `export * from ${JSON.stringify(entry)}\n`,
  )
  fs.writeFileSync(
    path.join(generatedDir, 'logging.d.ts'),
    [
      '// Generated by `1c assets` ([[REQ-235]]). Do not edit, do not commit.',
      '/* eslint-disable @typescript-eslint/no-explicit-any */',
      ...LOGGING_EXPORTS.map((n) => `export const ${n}: any`),
      '',
    ].join('\n'),
  )
  return entry
...
export async function buildControlAppAssets(repoRoot: string): Promise<AssetBuildReport> {
  ...
  const aiTicketingEntry = writeAiTicketingShim(generated)
  const loggingEntry = writeLoggingShim(generated)
...
export function formatAssetReport(report: AssetBuildReport): string {
  return [
    ...
    `tickets    ${report.aiTicketingEntry}`,
    `logging    ${report.loggingEntry}`,
    kbLine(report.kb),
    `out        ${report.outDir}`,
  ].join('\n')
}

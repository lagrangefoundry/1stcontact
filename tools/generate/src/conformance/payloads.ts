 ...
 * ([[DOC-20]] AC-M2) for **behavior modules**.
 *
 * The security dimension treats every behavior as the sanitization boundary for
 * untrusted input, so its payloads are derived **from the behavior's own
 ...
 * checks don't false-positive. Both satisfy the behavior contract (required
 ...
 */
import type { BehaviorMeta, BehaviorConfigSpec, BehaviorSlotValue } from '@1stcontact/framework'
...
function configValue(spec: BehaviorConfigSpec, hostile: boolean): unknown {
  switch (spec.type) {
    case 'url':
...
function buildSlots(meta: BehaviorMeta, hostile: boolean): Record<string, BehaviorSlotValue> {
  const slots: Record<string, BehaviorSlotValue> = {}
...
export function buildInjectionContent(meta: BehaviorMeta): {
...
export function buildBenignContent(meta: BehaviorMeta): {
  config: Record<string, unknown>
  slots: Record<string, BehaviorSlotValue>
} {
...
/**
 * A pair of fixtures for a behavior: the contract-derived injection fixture and
 * its benign twin, each carrying `config` + L1 `slots` props.
 */
export function buildSecurityFixtures(meta: BehaviorMeta): {

...
-- WHAT A CONTACT HAS AGREED TO, AS QUERYABLE STATE (REQ-240). The history is
-- `contact_events` above and is append-only; this is the current value, and
-- state is the thing that changes.
--
...
-- It is NOT a credential: it opens no session, names no membership, and reaches
-- nothing but one page and the artifacts that page lists. A sign-up link is the
-- opposite kind of thing — it creates a member, so it expires and is single-use,
-- which is what `login_tokens` further down this file already does. The two have
-- opposite rules and are deliberately two tables rather than one with a
-- `purpose` column, because one column is all it takes for somebody to make the
-- rules the same.
--
...
--
-- `business_id` IS DERIVED FROM THE CONTACT AND NEVER SUPPLIED, exactly as it is
-- on the two tables above: every write is `INSERT ... SELECT ... FROM users`, so
-- a grant cannot be filed under a business its contact does not belong to.
--
...
CREATE TABLE IF NOT EXISTS asset_grants (
  ...
  -- `memberships.revoked_at` below already uses. There is no operator surface
  -- for it yet; the gate enforces it, so the surface is additive when erasure
  -- arrives.
  revoked_at  TEXT,
  FOREIGN KEY (contact_id) REFERENCES users (id) ON DELETE CASCADE
);

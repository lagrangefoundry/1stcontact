...
-- `0020` has been applied, and `wrangler d1 migrations apply` records what it has
-- run, so an edit reaches no database. Same reasoning as `0005`, `0007`–`0009`,
-- `0013`, `0014`, `0016`, `0018`, `0019` and `0020`.
--
-- WHY `0021` AND NOT `0020`: [[REQ-304]]'s `0020_asset_digest.sql` landed on the
-- working branch while this one was being written. `wrangler` orders migrations
-- by FILENAME, so two files sharing a number is an ordering it cannot resolve —
-- the later ticket renumbers rather than the earlier one being moved, because
-- `0020_asset_digest.sql` may already have been applied somewhere and a file
-- that has run cannot be renamed without the recorded name ceasing to match.
--

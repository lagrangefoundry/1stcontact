 ...
 * admitted and already scoped to one business.
 *
 * THERE IS NO CEILING ON HOW OFTEN A CONVERSATION MAY LOOK, and its absence is
 * deliberate ([[REQ-286]] supersedes REQ-206's rate-limiting decision; the rest
 * of REQ-206 stands). A session quota was here, denominated in browser
 * acquisitions, and it was metering the wrong currency: a capture costs eight
 * acquisitions and hands back a name, while a `screenshot` costs none and puts
 * an image into the conversation that every later turn pays for again. What is
 * scarce is CONTEXT, not page loads. Authorisation is still answered by there
 * being no route; spend is answered in tokens, at the point of call, in
 * `fidelity-surface.json`. If browser spend ever does become a problem it comes
 * back as a rate limit with the evidence attached, not as a session quota.
 */
 ...
 *
 * AND NOTHING METERS IT ([[REQ-286]]). A per-session ration of browser
 * acquisitions used to wrap the launcher here; it was removed on purpose rather
 * than raised, because it charged for the cheap thing and let the expensive one
 * through free. Authorisation is still answered by the paragraph above. Spend is
 * answered where it is actually incurred — in tokens, stated at the point of
 * call — so the launcher below is just the launcher.
 *
...
export function fidelityDeps(
  ...
  //
  // ONE LAUNCHER, SHARED BY BOTH FACTORIES, and nothing counts what goes through
  // it ([[REQ-286]]).

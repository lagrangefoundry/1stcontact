    ...
    if (p === '/api/publish' && method === 'POST') {
      ...
      if ((request.headers.get('accept') ?? '').includes('text/event-stream')) {
        return streamPublish(
          (onLadderProgress) =>
            publishSite(store, slug, { message, ladder, templateRefusal, onLadderProgress }),
          answer,
          scrub,
        )
      }

      return json(200, await answer(await publishSite(store, slug, { message, ladder, templateRefusal })))
    }
